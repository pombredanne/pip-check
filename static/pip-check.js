	...
	$(document).on("click", 'a.update', function(event) {
		...
		$.post(event.target.href, function(data) {
			if (data === "") {
				...
				$('li#'+event.target.href.split('/')[event.target.href.split('/').length-1].split("==")[0]).children('.version').text(event.target.href.split("==")[1]);
				$('li#'+event.target.href.split('/')[event.target.href.split('/').length-1].split("==")[0]).css('background-color', '#85C57C');
